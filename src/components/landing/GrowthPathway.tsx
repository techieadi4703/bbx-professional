  ...
  const tiers = [
    {
      ...
      style: "bg-surface-container-lowest border-border text-on-surface"
    },
    {
      name: "Prime",
      icon: Shield,
      ...
      style: "bg-surface-container-lowest border-secondary text-on-surface shadow-lg relative z-10 scale-100 md:scale-105"
    },
    {
      name: "Elite",
      icon: Trophy,
      ...
      style: "bg-secondary border-secondary text-secondary-foreground"
    }
  ];
        ...
        <Reveal width="100%">
          <div className="mb-16 text-center">
            <h2 className="text-3xl md:text-5xl font-headline tracking-tight text-on-surface">
              Grow from <span className="italic text-secondary">Classic to Elite</span>
            </h2>
            <p className="mt-4 text-on-surface-variant max-w-2xl mx-auto font-medium">
              ...
              <RevealItem key={i} className="h-full">
                <div className={`p-4 md:p-10 rounded-2xl border-2 h-full flex flex-col transition-all ${tier.style}`}>
                  <div className="mb-4 md:mb-8">
                    <tier.icon className={`w-8 h-8 mb-3 md:w-10 md:h-10 md:mb-6 ${tier.name === 'Elite' ? 'text-primary-foreground/80' : 'text-secondary'}`} />
                    <h3 className="font-headline text-xl md:text-3xl italic font-bold mb-1 md:mb-2">{tier.name}</h3>
                    <p className={`text-[10px] sm:text-xs font-bold uppercase tracking-widest ${tier.name === 'Elite' ? 'text-primary-foreground/70' : 'text-on-surface-variant'}`}>
                      {tier.requirements}
                    </p>
                  </div>
                  
                  <ul className={`space-y-2 md:space-y-4 flex-grow ${tier.name === 'Elite' ? 'text-secondary-foreground/90' : 'text-on-surface-variant'}`}>
                    {tier.perks.map((perk, j) => (
                      <li key={j} className="flex items-start gap-2.5 md:gap-3">
                        <span className={`w-1.5 h-1.5 rounded-lg mt-2 shrink-0 ${tier.name === 'Elite' ? 'bg-surface-container-lowest' : 'bg-secondary'}`} />
