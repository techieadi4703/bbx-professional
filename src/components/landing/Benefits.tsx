      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-12">
        <Reveal width="100%">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {benefits.map((b, i) => (
              <RevealItem key={i}>
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-[#e5e2df] hover:border-[#735c00] transition-colors h-full flex flex-col">
                  <div className="w-12 h-12 rounded-full bg-[#735c00]/10 flex items-center justify-center mb-6 text-[#735c00]">
                    <b.icon className="w-6 h-6" />
                  </div>
                  <h3 className="font-headline text-2xl font-bold text-[#1c1c1a] mb-3">{b.title}</h3>
                  <p className="text-[#74777d] font-medium leading-relaxed flex-grow">
                    {b.body}
                  </p>
                </div>
              </RevealItem>
